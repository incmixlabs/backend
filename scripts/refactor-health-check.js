...
if (!fs.existsSync(healthCheckDir)) {
  console.error(`Error: Health check directory not found in ${serviceName} service`);
  process.exit(1);
}
...
if (!envVarsMatch) {
  console.error('Error: Could not extract environment variables from env-vars.ts');
  process.exit(1);
}
