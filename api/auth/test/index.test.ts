...
describe("Auth API tests", () => {
  ...
  test("signup new user invalid email", async () => {
    const res = await app.request(`${BASE_PATH}${signup.getRoutingPath()}`, {
      method: "POST",
      headers: jsonDefaultHeaders,
      body: JSON.stringify({
        fullName: "John Doe",
  ...
  test("login with incorrect email", async () => {
    const res = await app.request(`${BASE_PATH}${login.getRoutingPath()}`, {
      method: "POST",
      headers: jsonDefaultHeaders,
      body: JSON.stringify({
  ...
  test("delete user", async () => {
    ...
    expect(reLoginRes.status).toBe(reLoginRes.status)
  })
})
