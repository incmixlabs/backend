...
test.describe("Files API", () => {
  ...
  test("Authentication requirement - should reject unauthenticated requests", async ({
    request,
  }) => {
  ...
  test("File upload - should generate presigned URL and handle file upload", async ({
    request,
  }) => {
  ...
  test("File download - should retrieve uploaded file content", async ({
    request,
  }) => {
  ...
  test("File deletion - should delete file and make it unavailable for download", async ({
    request,
  }) => {
    ...
    const presignedDeleteRes = await request.get(
      "/api/files/presigned-delete",
      {
        params: { fileName },
        headers: { cookie: cookie as string },
      }
  ...
  test("Error handling - should return appropriate error for non-existent file", async ({
    request,
  }) => {
    const { downloadRes } = await downloadTestFile(
      request,
